...
## Как запустить быстрый прототип
1. **Создайте `.env` файлы** (пример значений):
   - `apps/api/.env`
     ```env
     PORT=3001
     POSTGRES_URL=postgresql://cex:cex@db:5432/cex
     REDIS_URL=redis://redis:6379
     JWT_SECRET=change_me
     ADMIN_DEFAULT_EMAIL=admin@example.com
     ADMIN_DEFAULT_PASSWORD=Admin123!
     ```
   - `apps/user-frontend/.env.local`
     ```env
...
2. **Структура директорий** (минимум для запуска):
   ```
   apps/
     api/                # NestJS backend
     admin-frontend/     # Next.js admin
     user-frontend/      # Next.js user app
   docker-compose.yml
   ```

3. **Пример `docker-compose.yml`** (упрощённый):
   ```yaml
   version: "3.9"
   services:
     db:
...
4. **Команды для запуска** (в корне):
   ```bash
   docker compose up --build
   ```
   - Пользовательская часть: http://localhost:3000
   - Админка: http://localhost:3002 (логин admin@example.com / Admin123!)

5. **Первичная миграция/сидинг** (если используете Prisma или TypeORM):
   ```bash
   docker compose exec api npm run prisma:migrate        # или npm run typeorm:migrate
   docker compose exec api npm run seed:admin           # создаёт дефолтного админа
   ```

6. **Фоновый воркер** (депозиты/ордера):
   - Либо отдельный сервис `worker` в compose, либо запуск команды в контейнере API:
     ```bash
...
7. **Проверка здоровья сервисов**:
   ...
   - Postgres: `docker compose exec db psql -U cex -d cex -c "\dt"`
